...
beta = ((lticks1 - lticks0) - (alpha_l * leftcodewheel_countperrev * M)) / \
    ((rticks1 - rticks0) - (alpha_r * leftcodewheel_countperrev * M))
...
radius_l = -(D * (leftcodewheel_countperrev + rightcodewheel_countperrev) /
             2) / (math.pi * (beta * (rticks1 - rticks0) + (lticks1 - lticks0)))
radius_r = beta * (-radius_l)
axletrack = alpha_r * radius_r + alpha_l * radius_l
wheeledbase.set_parameter_value(ODOMETRY_AXLETRACK_ID, axletrack, FLOAT)
